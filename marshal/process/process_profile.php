<?php
// marshal/process/process_profile.php
session_start();
...
switch ($action) {
    case 'update_profile':
        ...
        try {
            ...
            // Check if marshals record exists
            $check_marshal_sql = "SELECT id FROM marshals WHERE user_id = ?";
            $check_marshal_stmt = $pdo->prepare($check_marshal_sql);
            $check_marshal_stmt->execute([$user_id]);
            
            if ($check_marshal_stmt->rowCount() > 0) {
                // Update existing marshal record
                $marshal_sql = "UPDATE marshals SET station_point = :station_point, shift = :shift 
    ...
    case 'upload_image':
        ...
        try {
            ...
            $filename = 'marshal_' . $user_id . '_' . time() . '.' . $extension;
